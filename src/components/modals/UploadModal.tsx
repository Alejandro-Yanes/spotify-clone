...
const UploadModal: React.FunctionComponent<UploadModalProps> = (props) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (values) => {
    try {
      ...
      if (supabaseError) {
        setIsLoading(false);
        return toast.error(
          `Failed uploading image , Detailed Error : ${supabaseError?.message}`
        );
      }
    ...
    <div>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div>
            <div className="pb-1">Select a image for the song</div>
            <Input
              id="image"
              type="file"
              disabled={isLoading}
